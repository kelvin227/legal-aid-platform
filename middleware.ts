...
const PUBLIC_PATHS_LOGIN = ["/login", "signin"];
export default auth(async (req) => {
  ...
  if (!subdomain) {
    ...
    if (isPublicPath) {
      return NextResponse.redirect(
        new URL(`http://app.${host}${pathname}`, req.url)
      );
    }
  ...
  switch (subdomain) {
    case "app-lap": //case "app":
    {
      ...
      const isPublic_paths_login = PUBLIC_PATHS_LOGIN.includes(pathname);
      const isLoggedin = req.auth;
      if (!isLoggedin && isProtectedPath) {
        return NextResponse.redirect(new URL(`/login`, req.url));
      }

      if (isLoggedin && isPublic_paths_login) {
    ...
    case "web-lap": //    case "web":
    {
      const PROTECTED_PATHS = [
        ...
        "/tasks",
        "/Case",
      ]; //add all exact protected paths here
      const isProtectedPath = PROTECTED_PATHS.includes(pathname);
      const isLoggedin = req.auth;
      const isPublic_paths_login = PUBLIC_PATHS_LOGIN.includes(pathname);
      if (!isLoggedin && isProtectedPath) {
      ...
      if (isLoggedin && isPublic_paths_login) {
        return NextResponse.redirect(new URL(`/dashnoard`, req.url));
      }
      return NextResponse.rewrite(
        new URL(`/web${req.nextUrl.pathname}`, req.url)
      );
    }
    case "admin-lap": //case "admin":
    {
    ...
    case "api": {
      return NextResponse.rewrite(
        new URL(`/api${req.nextUrl.pathname}`, req.url)
      );
    }
